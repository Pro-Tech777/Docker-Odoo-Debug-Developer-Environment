...
List of your files and folders. Column headers with buttons are sortable. This list is not fully rendered for performance reasons. The files will be rendered as you navigate through the list.
Debug Odoo Using Docker.md





Below is an updated step-by-step guide for debugging Odoo using Docker, now including the installation of Docker, Docker Compose, and all Docker CLI tools as part of the prerequisites. The checklist for each step has been expanded accordingly.
...
Download and install Docker Desktop (MacOS/Windows) or Docker Engine (Linux) from the official Docker installation page: Docker Install.
...
docker buildx version
 If not installed, follow Buildx setup.
...
docker scan --version
 If unavailable, follow Docker Scan setup.
...
Clone the tutorial repository into $HOME/odoo/docker-odoo-debug:



cd $HOME/odoo && git clone https://github.com/Pro-Tech777/Docker-Odoo-Debug-Developer-Environment.git
Alternatively, download the files manually from the repository and place them in $HOME/odoo/docker-odoo-debug.

Checklist: 

Tutorial repository cloned or downloaded to $HOME/odoo/docker-odoo-debug.
...
Tutorial files cloned/downloaded to $HOME/odoo/docker-odoo-debug.
